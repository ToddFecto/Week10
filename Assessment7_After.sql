/* create the database - orgchartAfter */
create database orgchartAfter;
use orgchartAfter;
...
/* select distinct cities except null and output to view_cities */
select distinct(city) view_cities from persons where city is not null;
/* Actually what is needed to create a view - not just select data from exiting tables */
...
/* select/join persons first name, last name and job/role title */
